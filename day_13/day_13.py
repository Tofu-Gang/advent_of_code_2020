...
def puzzle_1() -> None:
    ...
    The answer should be 2238.
    """

    with open("day_13/input.txt", 'r') as f:
        lines = [line.strip() for line in f.readlines()]
        estimate = int(lines[0])
        ids = [int(id) for id in lines[1].split(',') if id.isnumeric()]
        min_depart_time = maxsize
        depart_id = -1

        for id in ids:
            depart = 0
            while depart < estimate:
                depart += id

            if depart < min_depart_time:
                min_depart_time = depart
                depart_id = id

        print(depart_id * (min_depart_time - estimate))
...
def puzzle_2() -> None:
    ...
    with open("day_13/input.txt", 'r') as f:
        lines = [line.strip() for line in f.readlines()]
        bus_ids_string = [bus_id for bus_id in lines[1].split(',')]
        bus_ids = [(int(bus_ids_string[i]), i)
                   for i in range(len(bus_ids_string))
                   if bus_ids_string[i].isnumeric()]
        timestamp = 0

        while any([(timestamp + bus_id[1]) % bus_id[0] != 0
